"""Create a class Inventory. The __init__ method should accept only the
__capacity: int (private attribute) of the inventory. You can read more about private attributes here.
...
class Inventory:
    ...
    def __repr__(self):
        string_to_output = f"Items: {', '.join(Inventory.items)}.\n"
        string_to_output += f"Capacity left: {self.get_capacity() - len(Inventory.items)}"

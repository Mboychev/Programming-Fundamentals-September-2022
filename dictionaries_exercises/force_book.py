"""The force users struggle to remember which side is the different force users from because they
...
•	If there is no such force user and no such force side -> create a new force side and add the
    ...
    if " | " in command:
        ...
        for value in side.values():
            if force_user in value:
                ...
                break
        if not user_in_here:
            if force_side not in side.keys():
                side[force_side] = [force_user]
            else:
                side[force_side].append(force_user)
...
for current_side in side.keys():
    if len(side[current_side]) > 0:
        print(f"Side: {current_side}, Members: {len(side[current_side])}")
    for force_user1 in side[current_side]:
        print(f"! {force_user1}")